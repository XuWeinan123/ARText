...
class MyARView: ARSCNView {
    ...
    func creatObject(at point: CGPoint) -> CreatObject? {
        let hitTestOptions: [SCNHitTestOption: Any] = [.boundingBoxOnly: true]
        let hitTestResults = hitTest(point, options: hitTestOptions)
        
        return hitTestResults.lazy.flatMap { result in
            return CreatObject.existingObjectContainingNode(result.node)
        }.first
    }

    /**
     从所点击的屏幕位置进行测试，以返回最可能的结果。
     返回新的world position，如果一个被击中，一个锚，如果被认为是在一个平面上。
...
extension MyARView.FeatureHitTestResult {
    ...
    init(featurePoint: float3, ray: MyARView.HitTestRay) {

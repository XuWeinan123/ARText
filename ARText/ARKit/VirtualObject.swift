...
class VirtualObject: SCNReferenceNode {
    ...
    func adjustOntoPlaneAnchor(_ anchor: ARPlaneAnchor, using node: SCNNode) {
        // Get the object's position in the plane's coordinate system. 得到物体在平面坐标系统中的位置。
        let planePosition = node.convertPosition(position, from: parent)
        
        // Check that the object is not already on the plane. 检查物体是否已经在平面上。
        guard planePosition.y != 0 else { return }
        
        // Add 10% tolerance to the corners of the plane. 在平面的角上增加10%的公差。
